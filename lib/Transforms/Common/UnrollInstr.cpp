...
struct UnrollInstrPattern : OpRewritePattern<hw::HWModuleOp> {
  ...
  LogicalResult matchAndRewrite(hw::HWModuleOp top,
    ...
    DelayOp delta = builder.create<DelayOp>(inst.getType(0), inst.getResult(0),
                                            enable, inst.getResult(0),
    ...
    for (size_t i = 1; i < cons.size(); i++) {
      ...
      delta = builder.create<DelayOp>(inst.getType(0), inst.getResult(0),
                                      enable, inst.getResult(0),
                                      builder.getI32IntegerAttr(1));
    }
