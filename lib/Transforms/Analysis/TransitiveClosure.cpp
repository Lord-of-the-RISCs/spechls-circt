...
void PathAnalyzer::buildAdjacencyMatrix(
    std::function<bool(mlir::Operation *)> filter) {
...
void PathAnalyzer::computeTransitiveClosure() {
  ...
  for (size_t k = 0; k < n; ++k) {
    for (size_t i = 0; i < n; ++i) {
      for (size_t j = 0; j < n; ++j) {
        forwardMatrix[i][j] =
            forwardMatrix[i][j] || (forwardMatrix[i][k] && forwardMatrix[k][j]);
        backwardMatrix[i][j] = backwardMatrix[i][j] ||
                               (backwardMatrix[i][k] && backwardMatrix[k][j]);
      }
    }
  }
}
...
PathAnalyzer::PathAnalyzer(Region &region, bool intraIteration)
  ...
  forwardAdjMatrix =
      new std::vector<llvm::BitVector>(numOps, llvm::BitVector(numOps, false));
...
bool PathAnalyzer::isTransitiveSuccessor(mlir::Operation *op,
                                         mlir::Operation *succ) {
...
bool PathAnalyzer::isTransitivePredecessor(mlir::Operation *op,
                                           mlir::Operation *pred) {
  int j = opToIndexMap[op];
...
PathAnalyzer::getTransitiveSuccessors(mlir::Operation *op) {
...
llvm::SmallVector<mlir::Operation *>
PathAnalyzer::getTransitivePredecessors(mlir::Operation *op) {
  llvm::SmallVector<mlir::Operation *> res;
  std::vector<llvm::BitVector> &backwardMatrix = *backwardAdjMatrix;
  std::vector<llvm::BitVector> &forwardMatrix = *forwardAdjMatrix;
